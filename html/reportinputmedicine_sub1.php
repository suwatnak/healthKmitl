...
$medicinereport = $_GET['medicinereport'];
                        $start = $_GET['start'];
                        $end = $_GET['end'];
                                        echo '<thead><tr><th class="center"><label class="pos-rel"><input type="checkbox" class="ace" /><span class="lbl"></span></label></th><th>รหัสยา</th> <th>ชื่อยา</th> <th>จำนวน</th><th>หน่วย</th><th>วัน</th><th>เวลา</th></tr></thead>';
                                        if ($medicinereport == '0') {
...
WHERE  DATEE BETWEEN TO_DATE('$start', 'dd/mm/yyyy') AND  TO_DATE('$end', 'dd/mm/yyyy') ORDER BY DATEE ";
                                            $objParse = oci_parse($objConnect, $strSQL);
                                            oci_execute($objParse);
                                            while (oci_fetch($objParse)) {
                                                echo '<tbody><tr><td class="center"> <label class="pos-rel"><input type="checkbox" class="ace" /><span class="lbl"></span></label> </td><td class="center">' . oci_result($objParse, 'MEDICINEID') . '</td><td>' . oci_result($objParse, 'NAME') . '</td><td>' . oci_result($objParse, 'QUANTITY') . '</td><td class="center"><span class="label label-sm label-warning">' . oci_result($objParse, 'UNIT') . '</span></td><td>' . oci_result($objParse, 'DATEE') . '</td><td>' . oci_result($objParse, 'TIME') . '</td></tr></tbody>';
                                            }
...
SELECT * FROM(
SELECT A_INPUTMEDICINE.MEDICINEID,A_MEDICINE.NAME,A_INPUTMEDICINE.QUANTITY,A_MEDICINE.UNIT,TO_DATE(A_INPUTMEDICINE.DATEE, 'dd/mm/yyyy') AS DATEE,A_INPUTMEDICINE.TIME FROM A_INPUTMEDICINE INNER JOIN A_MEDICINE ON A_INPUTMEDICINE.MEDICINEID=A_MEDICINE.MEDICINEID)
WHERE  MEDICINEID='$medicinereport' AND DATEE BETWEEN TO_DATE('$start', 'dd/mm/yyyy') AND  TO_DATE('$end', 'dd/mm/yyyy') ORDER BY DATEE";
                                            $objParse = oci_parse($objConnect, $strSQL);
                                            oci_execute($objParse);
                                            while (oci_fetch($objParse)) {
                                                echo '<tbody><tr><td class="center"> <label class="pos-rel"><input type="checkbox" class="ace" /><span class="lbl"></span></label> </td><td class="center">' . oci_result($objParse, 'MEDICINEID') . '</td><td>' . oci_result($objParse, 'NAME') . '</td><td>' . oci_result($objParse, 'QUANTITY') . '</td><td class="center"><span class="label label-sm label-warning">' . oci_result($objParse, 'UNIT') . '</span></td><td>' . oci_result($objParse, 'DATEE') . '</td><td>' . oci_result($objParse, 'TIME') . '</td></tr></tbody>';
