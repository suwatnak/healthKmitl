...
if ($functionName == "chacknamelastnameperson") {
    chacknamelastnameperson($namestudent,$lastnamestudent);
...
function insertdataA_PATIENT($codestudent, $namestudent, $lastnamestudent, $gender, $faculty) {
    ...
    VALUES (A_PERSONID_AUTO.NEXTVAL, '$namestudent', '$lastnamestudent', '$gender', '$faculty','บุคลากร')";
    $objParse = oci_parse($objConnect, $strSQL);
    oci_execute($objParse);
    oci_close($objConnect);
}

function chacknamelastnameperson($namestudent,$lastnamestudent) {
    require("db_connect.php");
    $strSQL = "SELECT * FROM A_PATIENT WHERE TNAME='$namestudent' AND LNAME='$lastnamestudent' AND TYPE='บุคลากร'";
    $objParse = oci_parse($objConnect, $strSQL);
    oci_execute($objParse);
    header('Content-Type: application/json');
